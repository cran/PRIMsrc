...
This branch (master) is the  default one, that hosts the current development release (version 0.7.4) of the survival bump hunting procedure 
that implements the case of a survival response. Note that `PRIMsrc` is still a non-production release and that version 0.7.4 implements 
...
PRIMsrc (>= 0.7.4) requires R-3.0.2 (2013-09-25). It was built and tested under R version 3.4.1 (2017-06-30) and Travis CI. 
...
* Alternatively, you can install the most up-to-date development version (>= 0.7.4) of `PRIMsrc` from the [GitHub](https://github.com/jedazard/PRIMsrc) repository, 
